...
// (NOT cascade-deleted, NOT orphaned). If the deleted node was the root
// (no parent), its children become new roots (level 0) of their own.
// Any cross-links touching the deleted node are removed.
function deleteNode(db, nodeId) {
  const node = db.prepare('SELECT * FROM nodes WHERE id = ?').get(nodeId);
  if (!node) throw new Error('Node not found');

  const newParentId = node.parent_id; // null if node was root
  const newParentLevel = newParentId
    ? db.prepare('SELECT level FROM nodes WHERE id = ?').get(newParentId).level
    : -1; // so children become level 0
